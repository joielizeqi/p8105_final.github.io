...
To further increase the precision of the model fit, interaction terms between variables were added. After using stepwise reduction to simplify the model complexity, the final negative binomial regression model is as follows.
...
```
最终筛选得到的模型，主效应包括：`year` `month` `gender` `spay_neuter` `borough`。
* 双重交互效应包括：year:spay_neuter：年份与绝育状态的交互。
`year:borough`：年份与地区的交互。
`month:spay_neuter`：月份与绝育状态的交互。
`gender:spay_neuter`：性别与绝育状态的交互。
`gender:borough`：性别与地区的交互。
`spay_neuter:borough`：绝育状态与地区的交互。
总结：最终得到的model包含了
...
print(filter_model_summary)

```

2. 交互效应可视化：

使用最终模型进行交叉验证。图中红色虚线表示理想状态下的预测情况（即预测值等于实际值）。若点落在这条线附近，说明模型预测效果较好。若点的位置高于对角线，则模型低估了实际值；反之若点的位置低于对角线，则模型高估了实际值。
...
此外，还可以通过残差模拟和诊断来评估拟合模型的质量。

1. Residuals vs Fitted (残差与拟合值图)
用于检查拟合值和残差之间是否存在系统性关系。理想情况下，残差应随机分布在横轴（拟合值）附近，无明显模式，即红色虚线（平滑曲线）应接近水平直线。如果有明显模式（如曲线形分布），可能表示模型拟合不足。

2. Q-Q Residuals (残差的 Q-Q 图)
检查残差是否符合模型假设的分布。残差分布的尾部偏离正态分布。

3. Scale-Location (缩放位置图)
检查残差的方差是否均匀（同方差性）。点的分布呈现条带状，且红色平滑线显示出非水平趋势。这表明残差方差可能随拟合值变化，存在异方差性。

4. Residuals vs Leverage (残差与杠杆值图)
检查是否存在高杠杆值的观测点（对模型拟合影响较大的点）。理想情况:大多数点应集中在低杠杆值区域。没有点超出 Cook’s 距离（虚线表示）。
...
Q-Q 图和残差分布图表明残差的分布偏离正态分布，可能需要对响应变量（如 bite_count）进行变换（如对数变换）。
异方差性:

Scale-Location 图显示残差的方差随拟合值变化，可能需要引入更灵活的分布（如负二项分布或零膨胀模型）。
高杠杆点和异常点:

Residuals vs Leverage 图显示存在高杠杆点（如 832 和 637）。需要检查这些点是否数据录入错误，或者是否应在模型中单独处理。
改进模型建议:
检查分布假设:

如果模型使用的是泊松回归（GLM），可能需要切换到负二项分布（如 glm.nb()）。
使用残差模拟工具（如 DHARMa）进一步诊断残差分布。
调整异方差问题:

引入加权最小二乘回归或重新指定响应变量的分布。
处理异常点:

逐一检查高杠杆点和残差异常点，考虑是否需要移除或调整。
